...
\noindent
This chapter will provide case examples demonstrating both the influential applications and potential misuses of data visualisations, with the aim of underscoring the enduring importance of data visualisation. Following this, an introduction to the R package \textit{ggplot2} will be provided, along with a presentation of the datasets used in this thesis, and an overview of its organization.
...
\subsection{Biplots}
Biplots serve as influential visual tools for representing multivariate data in two dimensions, enabling the simultaneous visualization of both observations and variables in a dataset. They are designed to project high-dimensional data onto a lower-dimensional space while retaining the essential information present in the original dataset.

For the creation of a biplot, a matrix of observations is utilized, where rows denote individual cases and columns represent variables. These variables should ideally be quantitative and on the same scale to avoid biases arising from differences in magnitude. Additionally, the absence of perfect correlation among variables is assumed to prevent numerical instability or distorted representations.

The construction of a biplot involves mathematical computations often derived from techniques like Singular Value Decomposition (SVD) or Principal Component Analysis (PCA). These methods facilitate the reduction of dimensions while preserving as much variance as possible.
...
Where \( x_{i1} \) and \( x_{i2} \) are the coordinates of observation \( i \) on the first and second principal components respectively, \( s_{i1} \) and \( s_{i2} \) are the scores of observation \( i \) on these components, and \( u_{1} \) and \( u_{2} \) are the unit eigenvectors corresponding to the first and second principal components.
...
Here, \( v_{j1} \) and \( v_{j2} \) represent the coordinates of variable \( j \) on the first and second principal components respectively, \( \lambda_{1} \) and \( \lambda_{2} \) are the eigenvalues corresponding to the first and second principal components, and \( c_{j1} \) and \( c_{j2} \) are the loadings of variable \( j \) on these components.

In summary, biplots provide a geometrically intuitive way to visualize multivariate data in reduced dimensions, relying on mathematical techniques such as PCA for their construction.
