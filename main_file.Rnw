...
<<data-plots, echo=FALSE, message=FALSE, fig.height=4, fig.width=5, fig.pos = 'H', fig.cap='Different types of visualisation techniques according to the data type'>>=
common_theme <- theme(
...
plot3 <-  ggplot(attendance_data, aes(x = week, y = attendance)) +
  ...
  theme_minimal() +
  scale_y_continuous(breaks = seq(20, 32, by = 2), labels = function(x) round(x))+
  common_theme  # Adjust the breaks and labels
...
\subsubsection{Case Example: Classifiication of penguins}
...
<<QQboxplots, echo=FALSE, fig.height=2.8, fig.width=3.4, fig.pos='h', fig.cap='Q–Q boxplot of iris dataset', message=FALSE,warning=FALSE>>=
my_palette <- c("#459395", "#EB7C69", "#FDA638")
qqplot <- iris %>%  
