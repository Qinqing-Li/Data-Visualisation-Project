...
R provides us with a dedicated package for t-SNE analysis, Rtsne, which allows us to conduct the analysis conveniently using the \textit{Rtsne} function. We will use \textbf{Palmer Penguin} dataset to make a tSNE plot in R.
