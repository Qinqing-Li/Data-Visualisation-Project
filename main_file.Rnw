...
\noindent 
Significant aspects of human visual perception within data visualisation encompass pattern recognition, adept at identifying trends, outliers, and relationships in data representations. Additionally, perceptual grouping, where visually similar elements are grouped together, influences the interpretation of data clusters and shapes. Moreover, the =hierarchy of perception dictates that certain visual attributes are processed more swiftly and effectively than others, such as color being processed faster than text, influencing the viewer's attention hierarchy \cite{dastani2002role}.\\
...
\begin{figure}[H]
<<message=FALSE,warning=FALSE,fig.dim = c(5, 3)>>=
...
\noindent Principal curves are a concept closely related to PCA but, they are more flexible and powerful in capturing the intrinsic structure of high-dimensional data. While PCA focuses on finding linear projections that maximize variance, principal curves seek to identify smooth, nonlinear trajectories that capture the most significant variations in the data. These curves are essentially one-dimensional paths embedded within the multidimensional space of the data points.\\

\noindent Principal curves find application in various domains, including dimensionality reduction, and exploratory data analysis. They can reveal the underlying structure and patterns in complex datasets, aiding in understanding the relationships between variables and identifying important features. Principal curves can be used for data preprocessing, clustering, classification, and regression tasks, offering insights into the data's intrinsic geometry and facilitating more accurate modeling and decision-making processes. 

\subsubsection{Construction of Principal Curves}

<<P-curves, fig.height=4, fig.width=9, fig.cap='Principal curve fitting illustration on artifical dataset', message=FALSE>>=
...
<<PCAbiplot, fig.height=5, fig.width=8, fig.cap='PCA Biplot of mtcars Dataset: R In-built Function', message=FALSE>>=
