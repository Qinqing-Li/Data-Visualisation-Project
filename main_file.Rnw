...
\subsection{Biplots}
\noindent 
Biplots serve as a valuable tool in visualising designed to project high-dimensional data onto a lower-dimensional space while retaining essential information. This technique, introduced by Herman Chernoff in the 1970s, aids in understanding the relationships and patterns within high-dimensional data. They are commonly understood to be a generalisation of the simple two-variable scatterplot.\\

\noindent At its core, a biplot offers a graphical representation a data matrix, whose rows \(n\) are the samples, and whose columns \(p\) are the variables, by projecting these onto a two-dimensional plane while preserving the most significant information.\\
