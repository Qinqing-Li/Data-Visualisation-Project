...
\noindent
However, using the animation function, we can see how life expectancy and GDP per capita change through the years for each country, so it helps to show how relationships between variables evolve over time or across different categories. \\
\\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{image_reference/gganimate.png}
