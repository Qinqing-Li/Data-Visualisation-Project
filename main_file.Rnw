...
\noindent
\textbf{dplyr}: This packages plays a crucial role in data manipulation and transformation. It helps us easily filter, sort, summarize, and transform datasets, making data analysis more efficient and manageable. \textbf{maps}: This package is primarily used for providing map data and tools, allowing you to easily draw and display maps in geospatial analyses. \textbf{tidyverse}: This package is indeed a collection of R packages designed to provide a cohesive and comprehensive set of tools for data science. It includes several widely used packages such as $ggplot2$ and $dplyr$. \textbf{gridExtra}: This packages offers the capability to combine several graphical objects into a cohesive display.\\
...
$$\mathrm{AIC} = -2 \l(\mathbf{\hat{\theta}})+2 \dim (\mathbf{\theta}) \cite{aic},$$
\\  
