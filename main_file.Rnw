...
A histogram is a visual representation of the distribution of a (numerical) dataset. It is composed by a series of contiguous rectangles, also known as ``bins", each representing a range of data values. The height of each of these corresponds to the frequency or count of data points within that range, depending on the type of histogram. On a cartesian coordinate system, typically, the x-axis shows the endpoints of each bin, and the y-axis represents count or frequency. Hence, histograms facilitate the visualisation the distribution, spread, and central tendency of datasets.

\subsubsection{Theory of Histogram Bin-Width Selection}

\noindent The classical frequency histogram is constructed by dividing the data into a series of nonoverlapping bins. The data points within each bin are then counted. Hence, the histogram can be fully described by two main factors: the bin width $h$, and the bin origin $t_{0}$. However, in order for the bin counts to be comparable, the bins should all have the same width.\\

\noindent Sturges' Rule, established by Herbert Sturges in 1926, offers a systematic approach to determining the optimal number of bins for constructing a frequency histogram \cite{scott2015multivariate}. It suggests setting the number of bins, denoted as $K$, to \[ k = 1 + \log_{2} n, \] where $n$ represents the sample size.\\

\noindent This rule is based on the concept of normality, with the binomial distribution serving as a model for an optimally constructed histogram. Sturges' observation links the binomial distribution with normally distributed data, providing a basis for selecting the number of bins to achieve a histogram resembling a normal density curve.\\
...
\noindent Histograms can be also used to visualise the convergence of sample means to the population mean. Define the sample mean \[ \bar{X} = \frac{1}{n}\sum_{i=1}^n X_i \]. 
\noindent Then, as \( n \) becomes large, the variable \[ Z = \frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \] approaches a standard normal distribution.\\
