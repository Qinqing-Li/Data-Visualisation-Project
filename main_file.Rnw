...
\noindent If most data falls into a linear subspace, PCA finds direction in the subspace of the 2-dimensional space that passes through the middle of the scatter plot ie. minimise the sum of square orthogonal deviations between the points and the projections onto the subspace.\\
...
<<P-curves-code, message=FALSE>>=
#PRINCIPAL CURVE FUCNTION

# Dataset x with two dimensions
...
\noindent The scatter plot displays a distribution characterised by non-linear relationships between the dimensions. The fitting of the principal curve reveals its ability to adapt to the form of the scatter plot, effectively capturing the underlying structure of the data. 
