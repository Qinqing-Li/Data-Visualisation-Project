...
\subsection{Biplots}
\noindent 
Biplots serve as a valuable tool in visualising multivariate data by displaying both sample points and variables simultaneously in a lower-dimensional space. That is, they are designed to project high-dimensional data onto a lower-dimensional space while retaining the essential information present in the original dataset. This technique, introduced by Herman Chernoff in the 1970s, is a geometric representation that aids in understanding the relationships and patterns within high-dimensional data.\\
...
\noindent The construction of a biplot involves mathematical computations often derived from techniques like Singular Value Decomposition (SVD) or Principal Component Analysis (PCA). These methods facilitate the reduction of dimensions while preserving as much variance as possible.\\

\noindent SVD breaks down the data matrix into three constituent matrices, allowing for the extraction of singular values and singular vectors. These singular vectors form the basis for plotting the data in the reduced-dimensional space.\\

\noindent In practical terms, the steps to generate a biplot involve:
\begin{itemize}
    \item \textbf{Data Standardisation}: Before creating a biplot, it's common to standardize the data by centering and scaling it to ensure each variable contributes equally.
    \item \textbf{Calculating Singular Value Decomposition}: Applying SVD to the standardized data matrix to obtain the singular values and vectors.
