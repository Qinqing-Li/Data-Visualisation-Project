...
<<fire-by-months fy22,fig.height=1.5>>=
...
<<fire-by-months fy13-22,fig.height=5>>=
...
<<>>=
# Calculate 7-day moving average for each currency
columns <- names(MyData)[!names(MyData) %in% "Date"]
for (col in columns) {
  new_col_name <- paste0(col, "_MA7")
  MyData[[new_col_name]] <- zoo::rollapply(MyData[[col]], width=7, FUN=mean, fill=NA, align='right')
}
...
ggplot(plot_data, aes(x=Date, y=Rate, color=Currency)) +
  ...
  labs(title="7-Day Moving Average of Exchange Rates",
       subtitle="",
       y="Exchange Rate to GBP (7-Day MA)",
       x="Date",
       color="Currency") +
  theme_minimal() +
...
<<>>=
# Convert Date column to Date type and create a frame column
MyData$Date <- as.Date(MyData$Date, format = "%d-%b-%y")
MyData$frame <- as.character(MyData$Date)

# Create the animated plot
plot_ly(data = MyData, x = ~Date, y = ~USDtoGBP, text = ~Date, ids = ~Date,
        frame = ~frame, type = 'scatter', mode = 'lines', line = list(width = 2)) %>%
