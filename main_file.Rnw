...
<<data-plots, echo=FALSE, message = FALSE, fig.height=4, fig.width=6, fig.cap='Different types of visualisation according to the data type used'>>=
...
plot3 <- ggplot(interval_data, aes(x = time, y = temperature)) +
  geom_line(linewidth = 0.7, color = my_palette[1]) +
...
plot4 <-ggplot(income_data, aes(x = income)) +
  geom_histogram(binwidth = 5000, fill = my_palette[3], color = my_palette[3], alpha = 0.5) +
...
clear_plot <- ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("setosa" = "grey", "versicolor" = "grey", "virginica" = "red")) + 
...
<<scatter-plot, echo=FALSE, fig.height=2, fig.width=4, fig.cap='Scatter plot of car weights vs MPG', message=FALSE>>=
...
<<buble-plot-chunk, echo=FALSE, fig.height=5, fig.width=10, fig.caption='Bubble plot illustrating the relationship between car models and miles per gallon'>>=
#Create bubble plot 
