...
\noindent
Using the same dataset, we explore the relations between maximum oil displacement (which is Engine Volume) and car efficiency (mearsured in Miles per Gallon), using two methods: LOESS and linear regression model. The LOESS model
