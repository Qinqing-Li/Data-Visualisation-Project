...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Screenshots of animated scatterplot at different years}
    \label{fig:overall}
\end{figure}

\noindent The animated scatter plots provide a dynamic visualisation of the relationship between GDP per capita and life expectancy in each country over time. This animation helps in understanding temporal trends and patterns in the data that may not be apparent in static plots.\\
