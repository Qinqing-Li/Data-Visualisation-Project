...
\noindent
\textbf{gapminder}: The built-in R dataset, \href{https://www.rdocumentation.org/packages/gapminder/versions/1.0.0}{gapminder}, was provided by Gapminder Foundation. It encapsulates key demographic statistics like life expectancy, GDP per capita, and population across various countries spanning multiple years, illustrating global development trends.\\
...
\noindent
Figure~\ref{fig:gapminder_static} portrays a static scatter plot demonstrating the relationship between gross domestic product (GDP) per capita and life expectancy across various countries. GDP per capita is depicted on a logarithmic scale along the x-axis to accommodate a wide range of values, while life expectancy is represented on the y-axis. Moreover, the color each of the dots indicate the country they represent data of.\\
...
\noindent
In this static scatter plot there appears to be a positive correlation between GDP per capita and life expectancy. That is, as GDP per capita increases, life expectancy behaves in a similar way. \\
