...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image_reference/UniversityLogo.png}
\end{figure}
...
\vspace {0.25cm}
\LARGE Undergraduate Mathematics Project \\ 
...
\begin{figure}[H]
    ...
    \caption{Incomplete Data Analysis, ``How to lie with statistics" by Miguel de Carvalho}
    \label{fig:misuse1}
\end{figure}
...
\noindent \textbf{Built-in R Datasets}\\
\noindent \textbf{mtcars}: The \href{https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/mtcars}{mtcars} dataset was extracted from the 1974 Motor Trend US magazine. It includes the fuel consumption and 10 aspects of automobile design for 32 automobiles (1973–74 models). \\

\noindent
\textbf{iris}: The \href{https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/iris}{iris} dataset was collected by Edgar Anderson. This dataset was famously used by British statistician Ronald Fisher to demonstrate linear discriminant analysis in 1936. It encompasses 5 flower characteristics for 3 types of iris, each with 50 samples. \\

\noindent 
\textbf{gapminder}: The gapminder dataset was provided by the Gapminder Foundation. It encapsulates key demographic statistics like life expectancy, GDP per capita, and population across various countries spanning multiple years, illustrating global development trends.\\

\noindent
\textbf{palmerpenguins}: The \href{https://www.rdocumentation.org/packages/palmerpenguins/versions/0.1.1}{palmerpenguins} dataset was provided by the Palmer Station Long Term Ecological Research. This dataset contains biometric measurements from three distinct penguin species inhabiting the Palmer Archipelago in Antarctica, namely the Adélie, Chinstrap, and Gentoo penguins.\\

\noindent
\textbf{ToothGrowth}: The \href{https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/ToothGrowth}{ToothGrowth} dataset measures the tooth growth of 60 guinea pigs. Each animal was either administered with vitamin C or orange juice.\\

\noindent \textbf{External Datasets}
\noindent
...
\noindent
\textbf{Metabolic Syndrome Data}: The oxygen saturation in blood, for a healthy population of 80 women and a diseased population of 35 women. These data were collected through a survey conducted in North-West Spain. The dataset is accessible via \href{https://cloud.r-project.org/web/packages/DATAstudio/DATAstudio.pdf}{DATAstudio}, a collection of datasets from research articles by Miguel de Carvalho.\\
...
\setsbsecprefix{Data Abstraction and Representation}
...
\noindent By harnessing the principles of human visual perception, applying insights from cognitive psychology cognitive psychology, and leveraging pre-attentive attributes, data visualisation designers can create visualisations that are not only aesthetically pleasing but also cognitively efficient.	
...
\noindent Furthermore, employing interactive features like tooltips\index{Visual complexity!tooltips} and drill-down functionality\index{Visual complexity!drill-down functionality} assists in providing additional information when required, reducing the density of static visualisations. A final approach involves the use of progressive disclosure, presenting complex information gradually, beginning with an overview and allowing users to explore details as needed.\\
...
<<barcharts, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=10, fig.pos = 'H', fig.cap='Visualising tooth growth by dosage: A comparison of grouped and stacked bar chart techniques' >>=
# average length
...
plot_dt <- MyData %>% dplyr::select(Date, CANtoGBP, EURtoGBP, USDtoGBP) %>% gather(key="Currency", value="Rate", -Date) 
...
plot_data <- MyData %>% dplyr::select(Date, CANtoGBP_MA49, EURtoGBP_MA49, USDtoGBP_MA49) %>% gather(key="Currency", value="Rate", -Date) %>%
...
<<gapminder_static, echo=FALSE, fig.height=3, fig.width=4, fig.cap='Static scatter plot deconstruction of the gapminder dataset', message=FALSE>>=
p <- ggplot(
  gapminder, 
  aes(x = gdpPercap, y=lifeExp, colour = country)
  ) +
...
<<t-SNE, echo=FALSE, fig.height=3, fig.pos = 'H', fig.width=5, fig.cap='t-SNE of Penguins Dataset', message=FALSE, warning=FALSE>>=
penguins %>% 
