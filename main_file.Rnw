...
\noindent
Data visualisation is an area of interest in numerous method-specific publications. One notable publication is the \href{https://www.tandfonline.com/journals/ucgs20}{Journal of Computational and Graphical Statistics}. This journal publishes ongoing research on the latest techniques in computational and graphical methods in statistics, encompassing data analysis and numerical graphical displays. \href{https://www.jstatsoft.org/index}{Journal of statistical software} publishes peer-reviewed articles about statistical software, together with the source code. These publications together reveal the vast potential for current and future research in this ever-evolving domain, particularly as our digital landscape and data sphere expand at an accelerated pace.\\
...
\noindent
As a powerful tool for statistical analysis and graphic creation, R offers a wide array of functionalities to cater to a range of data visualisation needs. We will first introduce the commonly used R package \textit{ggplot2}, then we will present key packages for state-of-the-art data visualisation.\\

\noindent
In data visualisation, \textit{ggplot2} is the main tool used to create plots capable of representing various types of datasets. The foundation of \textit{ggplot2} lies in the Grammar of Graphics, allowing the sequential construction of individual elements composing a graph, which are subsequently combined to create a unified graphical representation.\\

\noindent
The \textit{ggplot2} is an R package for creating statistical graphics, which stands out due to its underlying Grammar of Graphics. It is flexible and powerful, allowing for the creation of tailored graphics rather than being confined to pre-set options. Although there's a learning curve, ggplot2 is designed to be user-friendly with sensible defaults and an iterative approach to building plots. It emphasizes revealing the message within the data. Additionally, ggplot2 is structured to facilitate layered and annotated graphics that enhance data analysis, which is particularly beneficial for beginners and those new to data analysis.\\
...
\noindent\textbf{Key ggplot2 Packages}\\
\noindent
The R language is a powerful data analysis tool, and its power lies in the wide variety of R packages that help it achieve a wide variety of functions. Through out this project, we will use mant different packages. These packages significantly enhances the analysis and presentation of data. \\
...
\noindent
The \textit{tidyverse} package is indeed a collection of R packages designed to provide a cohesive and comprehensive set of tools for data science. It includes several widely used packages such as \textit{ggplot2} and \textit{dplyr}.

\noindent
The \textit{Rtsne} package allows us to conduct the analysis conveniently using the \textit{Rtsne} function,  which is particularly useful for visualizing high-dimensional data.

\noindent
The \textit{qqboxplot} package helps us to build QQ boxplot which combines both QQ plot and boxplot, offering a more comprehensive visual assessment tool. It  enhances traditional boxplot and QQ plot by integrating their advantages together.
...
<<data-plots, echo=FALSE, message=FALSE, fig.height=8, fig.width=11, fig.pos = 'H', fig.cap='Different types of visualisation techniques according to the data type'>>=
...
<<abs-plots, echo=FALSE, fig.height=4, fig.width=12, fig.pos = 'H', fig.cap='Mtcars dataset visualised on 3 different levels of abstraction'>>=
...
<<colour-plot, echo=FALSE, fig.height=3, fig.width=10, fig.pos = 'H', fig.cap='Colour perception of a heatmap for by a colour blind person'>>=
...
<<cogload-plot, echo=FALSE, fig.height=3, fig.width=10, fig.pos = 'H', fig.cap='High versus low cognitive load demand through the reduction of visual complexity'>>=
...
\noindent Histograms can be also used to visualise the convergence of sample means to the population mean. Define the sample mean \( \bar{X} = \frac{1}{n}\sum_{i=1}^n X_i \). Then, as \( n \) becomes large, the variable 
\[ Z = \frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \] approaches a standard normal distribution.\\

\noindent
\textbf{Classical frequency histogram}\\

\noindent The classical frequency histogram is constructed by dividing the data into a series of nonoverlapping intervals, called bins. The data points within each bin are then counted. The histogram can be fully described by two main factors: the bin width, $h$, and the bin origin, $t_{0}$.\\
...
<<barcharts, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3, fig.width=10, fig.pos = 'H', fig.cap='Visualising tooth growth by dosage: A comparison of grouped and stacked bar chart techniques' >>=
# average length
...
barchart_plot1 <- ggplot(average_lengths, aes(x = factor(dose), y = average_len, fill = supp)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("#459395", "#FDA638"))
...
color_palette <- c("#459395", "#EB7C69", "#FDA638") # Define color palette
@

<<all exchange rates, message=FALSE,warning=FALSE,fig.dim = c(5.8, 3.5), fig.pos='H', fig.cap='Daily (top) and 49-day moving average (bottom) exchange rates of CAN, EUR, USD to GBP'>>=
...
\noindent
R function \texttt{decompose()} is able to decompose the time series by additive model or multiplicative model. And ~\ref{fig:decomposition of time series} is the demonstration of decomposition.

<<message=FALSE,warning=FALSE,echo=FALSE>>=
# Decomposition of addictive time series model
...
<<decomposition of time series, message=FALSE,warning=FALSE, fig.pos = 'H',fig.dim = c(5, 3), fig.cap='Decomposition of addictive time series model of CNY to GBP exchange rates'>>=
...
<<ACF, echo=TRUE, message=FALSE, warning=FALSE, fig.pos = 'H', fig.dim = c(4, 3), fig.cap= 'Autocorrelation Function at lag h in days of CNY to GBP Exchange Rate'>>=
...
<<roc, message=FALSE, warning=FALSE, fig.height=3.5, fig.width=3.5, fig.pos = 'H', fig.cap='ROC Curves for Two Diagnostic Tests'>>=
# PLOT OF ROC CURVES
...
<<bubble-plot-construction, echo=FALSE, fig.height=3.5, fig.width=11, fig.cap='Partial scatter plot deconstruction of the mtcars dataset', message=FALSE>>= 
...
plot2 <- ggplot(mtcars, aes(x = rownames(mtcars), y = hp)) +
  geom_point(color = "#EB7C69", size =2) +
...
plot3 <- ggplot(mtcars, aes(x = mpg, y = hp)) +
  geom_point(color = "#FDA638", size =2) +
...
<<gapminder_static, echo=FALSE, fig.height=3, fig.width=4, fig.cap='staticl scatter plot deconstruction of the gapminder dataset', message=FALSE>>=
p <- ggplot(
  gapminder, 
...
\\
<<gganimate, echo=TRUE, fig.show='hide', results='hide', message=FALSE, warning=FALSE, error=TRUE>>=
...
<<bubble-plot, echo=FALSE, fig.height=5, fig.width=11, fig.pos = 'H', fig.cap='Bubble plot illustrating trends in car types and performance', message=FALSE>>=
ggplot(mtcars, aes(x = rownames(mtcars), y = mpg, size = hp, color = cyl)) +
  ...
  scale_color_gradient(low = "#91cacc", high = "#3e7071") +
  theme_minimal() +
...
correlation_matrix <- ggplot(correlation_matrix, aes(Var1, Var2, fill = Freq)) +
  ...
  scale_fill_gradient2(low = "#459395", mid = "white", high = "#EB7C69", 
...
ggplot(data = mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "#EB7C69", linewidth = 1) +  # Linear regression trend line
  labs(
    x = "Weight (1000 lbs)",
    y = "Miles per Gallon",
    color = "Cylinders"
  ) +
  scale_color_manual(values = c("4" = "grey", "6" = "#4B0082", "8" = "#459395")) +  # Custom color palette
  theme_minimal() 
@
...
estate <- read.csv("RealEstate.csv", header = FALSE, col.names = var_names)
...
<<estate-data-plot,fig.height=2.5,fig.width=3.5,fig.pos='H', fig.cap='Estate valuation using house age, linear regression and smooth regression methods', message=FALSE,warning=FALSE>>=
...
<<PCAbiplot, fig.height=5.5, fig.width=12, fig.cap='PCA biplots of mtcars dataset', message=FALSE>>=

#MANUAL CONSTRUCTION OF A BIPLOT IN R
...
#GENERATION OF A BIPLOT WITH R'S BUILT-IN FUNCTIONS
# Standardise the data
scaled_data <- scale(mtcars)
...
<<P-curves, echo=FALSE, fig.pos = 'H', fig.height=4, fig.width=9, fig.cap='Principal curve fitting illustration on artificial dataset', message=FALSE>>=
# Generate random values for t
t <- runif(100, -1, 1)

# Create the dataset x with two dimensions
x <- cbind(t, t^3) + rnorm(200, sd = 0.05)

# Rename the columns
colnames(x) <- c("dim1", "dim2")

# Set up a multi-paneled plotting window
par(mfrow = c(1, 2))

# Plot the first dataset
plot(x, main = "Artificial Dataset 1", xlab = "Dimension 1", ylab = "Dimension 2", col = "#459395")

# Fit principal curve to the dataset
fit <- principal_curve(x)

# Plot the principal curve along with the data points
plot(fit, main = "Principal Curve", xlab = "Dimension 1", ylab = "Dimension 2")
whiskers(x, fit$s, col = "#EB7C69") 
@
...
\noindent
To create a symmetrical joint probability distribution in the high-dimensional space, t-SNE averages the conditional probabilities $p_{j|i}$ and $p_{i|j}$\cite{vanderMaaten2008tsne}, resulting in:
    \[
...
<<t-SNE, echo=FALSE, fig.height=3, fig.pos = 'H', fig.width=5, fig.cap='t-SNE of Penguins Dataset', message=FALSE>>=
penguins %>% 
...
\noindent
In this chapter, we will introduce some state-of-the-art graphical statistical methods published in recent research. In Chapter 6.1, we will introduce the foundational visualisation methods box plots and Q–Q plots. Building on this, we will discuss the theory and implementation of functional box plots and Q–Q Box plots, as published in recent articles in the \textit{Journal of Computational and Graphical Statistics}.
...
<<box-plots, echo=FALSE, fig.height=5, fig.width=10, fig.pos='h', fig.cap='Comparison of Violin and Box Plots of Sepal Length by Species', message=FALSE>>=

# Define custom color palette
my_palette <- c("#459395", "#EB7C69", "#FDA638")

# Box Plot
boxplot <- ggplot(iris, aes(x = Species, y = Sepal.Length, fill = Species)) +
  geom_boxplot(outlier.shape = NA, fill = my_palette, color = "black", notch = TRUE) +
  xlab("Species") +
  ylab("Sepal Length") +
  theme_minimal()
...
# Arrange plots side by side
grid.arrange(boxplot, violin_plot, ncol = 2)
@

\subsubsection{Q–Q Plots in Practice}\\

\noindent Quantile-Quantile (Q–Q) plots represent a powerful and widely utilised approach in exploratory data analysis and statistical inference, offering insightful visualisations of the similarity between two probability distributions. In section, the essence of Q–Q plots, their generation, the information they convey, and their relevance in contemporary data visualisation methodologies are explored.\\
...
<<QQplots, echo=FALSE, fig.height=4, fig.width=8, fig.pos='h', fig.cap='Q–Q plot of mpg in mtcars dataset', message=FALSE>>=
...
# QQ plot for mpg in mtcars dataset
qqplot(qnorm(ppoints(length(mtcars$mpg))), mtcars$mpg, col = my_palette[1], pch = 19,
       xlab = "Theoretical Quantiles (Standard Normal)", ylab = "Sample Quantiles (mpg)")
qqline(mtcars$mpg, col = my_palette[2])

# QQ plot for Sepal.Length in iris dataset
qqplot(qnorm(ppoints(length(iris$Sepal.Length))), iris$Sepal.Length, col = my_palette[3], pch = 19,
       xlab = "Theoretical Quantiles (Standard Normal)", ylab = "Sample Quantiles (Sepal.Length)")
qqline(iris$Sepal.Length, col = my_palette[2])
...
Building upon the traditional box plot, which is designed to summarize scalar data by depicting their quantiles and outliers, functional box plot focuses on datasets where each observation is a function over a certain domain, such as time or space. Thus, functional box plot is a powerful visualisation technique that offers a visual representation of the central tendency, spread, and shape of a distribution for functional datasets. By employing concepts such as Band Depth (BD), functional box plots allow for the assessment of centrality, variability, and the detection of outliers within a dataset of functions. These plots provide insights into the median function, the spread of the data, and functional outliers that deviate significantly from the overall data structure.\\

\noindent
Therefore, functional box plot connects the intuitive visual appeal of box plots with the complex, multidimensional nature of functional data. This subsection aims to unravel the methodology behind functional box plots, elucidating their construction, interpretation, and application in real-world datasets, which enables a deeper understanding of the patterns and anomalies inherent in functional data.

\subsubsection{Theory of Funtional Box Plot}

The foundation of constructing a boxplot is the organisation of data. This process is straightforward for univariate data, where observations are ordered from the smallest to the largest value. However, for multivariate data, this task becomes significantly more complex, necessitating advanced methodologies to establish a meaningful order. In the context of multivariate data, various metrics known as data depth have been developed to assess the centrality (or depth) of an observation or its deviation (outlyingness) from the central tendency.\\

\noindent
In the functional data analysis, BD offers a systematic framework to rank a set of curves, facilitating the identification of observations that are central. This concept is instrumental in defining functional quantiles and in assessing the centrality or the extent of deviation of specific curves. BD aims to quantify the order among sample curves by computing their BD values and ranking them accordingly. Specifically, the BD for any given function $y(t)$ within the set $\{y_1(t),\dots,y_n(t)\}$ is calculated by examining all possible combinations of $j$ functions from the set, forming a band for each combination, and determining the proportion of these bands that completely encompass $y(t)$.\\

\noindent
Hence, consider a set of observed functions $\{y_1(t),\dots,y_n(t)\}$ defined over an interval $I$ in $\mathbb{R}$. The graph of a function $y(t)$ is represented as $G(y) = \{(t,y(t)) : t \in I\}$. Define $B(y_{i_1},\dots,y_{i_k}) = \{(t,x(t)): t \in I, \min_{r=1,\dots,k} y_{i_r}(t) \leq x(t) \leq \max_{r=1,\dots,k} y_{i_r}(t)\}$ as the band of curves $y_{i_1},\dots,y_{i_k}$ \cite{SunGenton2011FunctionalBoxplots}. Let $J$ denote the number of curves that define a band, which is chosen to be any value from 2 to $n$, inclusive.\\

\noindent
Then, for a stochastic process $Y(t)$ generating the observations $y_1(t),\dots,y_n(t)$, the population version of BD for a curve $y(t)$ relative to a probability measure $P$ is determined by the formula\cite{SunGenton2011FunctionalBoxplots}:

\[BD_{J}(y, P) = \sum_{j=2}^{J} BD^{(j)}(y,P)=\sum_{j=2}^{J} P\{G(y) \subset B(Y_1,\dots,Y_j\}, \]

\noindent
where $B(Y_1,\dots,Y_j)$ is a band delimited by $j$ random curves. Here, $BD^{(j)}(y,P)$ calculates the probability that the graph of $y(t)$ lies entirely within a band formed by any selection of $j$ functions. These bands are delineated by the minimum and maximum values at each point $t$ among the selected $j$ functions. This formulation of BD facilitates a nuanced understanding of curve centrality and variability within functional data, laying the groundwork for the construction of functional boxplots. Then we can define the sample version of $BD(j)(y,P)$, which is the fraction of the bands determined by $j$ different sample curves containing the whole graph of the curve $y(t)$ \cite{SunGenton2011FunctionalBoxplots}:
...
<<QQboxplots, echo=FALSE, fig.height=4, fig.width=8, fig.pos='h', fig.cap='Q-Q  boxplot of iris dataset', message=FALSE,warning=FALSE>>=
my_palette <- c("#459395", "#EB7C69", "#FDA638")
qqplot <- iris %>%  
  ggplot(aes(Species, Sepal.Length, fill = Species)) + 
  geom_qqboxplot(varwidth = TRUE, notch = TRUE) +
  scale_fill_manual(values = my_palette) + # This line sets the color palette
  ylab("Sepal Length") + 
  ggtitle("Sepal Length Q-Q Boxplot") +
  theme(
    plot.title = element_text(size=14, face="plain", hjust=0.5),
    axis.title.x = element_text(size=12),
    axis.title.y = element_text(size=12),
    legend.position = "none"  # This line removes the legend, if desired
  ) 
