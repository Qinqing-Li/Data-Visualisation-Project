...
\noindent
Locally estimated scatterplot smoothing (LOESS) is a local regression method. It is a non-parametric regression method that fits a line segment for each neighbourhood of data. Unlike GLM, the LOESS model does not provide a global function to fit the data; rather, it fits neighborhoods of the data.\\

\noindent
Suppose we have $n$ observations $(x_1,y_1) \dots (x_n,y_n)$ with explanatory variable $x_i$ and response variable $y_i$. For each point $(x,y)$ consider the neighbourhood to fit a line segment around it. The size of the neighbourhood indicates the number of data points used to fit the line segments and is often expressed as a fraction $\alpha$ of the sample data. For $\alpha \leq 1$, each  neighbourhood consists of $\frac{n}{\alpha}$ number of points. Each data point $(x_i, y_i)$ in the neighbourhood of point $(x,y)$ is assigned with a weight, ``importance" of the data point to the line segment. The weight of $(x_i,y_i)$ is denoted as $w_i(x)$, which is defined as:
...
\noindent
For $\alpha > 1$, for each point $(x,y)$ the neighbourhood include all points. The maximum distance threshold is assumed to be $\alpha^{\frac{1}{p}}$, for $p$ explanatory variables.\\

\noindent
The smoothness of the LOESS curve depends on $\alpha$, therefore $\alpha$ is called the ``smoothing parameter".\\
