...
\noindent \textbf{PCA and biplots}\\

\noindent The heuristic argument motivating PCA biplots is the following. Defining $\mathbf{B}^{\alpha}$, for $0 \leq \alpha \leq 1$, as a diagonal matrix who's elements are $b_1^{\alpha}, \ldots, b_r^{\alpha}$ with a similar definition for $\mathbf{B}^{1-\alpha}.$ Then, with the decomposition of the data found through SVD, the data can be further rewritten as
\[\mathbf{X} = \mathbf{U}\mathbf{B}^{\alpha}\math{B}^{1-\alpha}\mathbf{S}^{T} = \mathbf{U}\mathbf{B}\mathbf{S}^{T} = \mathbf{G}\mathbf{H}^{T},\]
