...
\noindent
A time series $\{X_t\}$ is a moving average process of order $q$(MA(q)) if it can be expressed as:
...
R provides us with a dedicated package for t-SNE analysis, \textit{Rtsne}, which allows us to conduct the analysis conveniently using the \textit{Rtsne} function. In the graph presented below, we observe a t-SNE visualisation of the penguin dataset.\\
