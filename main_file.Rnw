...
<<barcharts, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3, fig.width=10, fig.pos = 'H', fig.cap='Visualising tooth growth by gosage: A comparison of grouped and stacked bar chart techniques' >>=
# average length
...
\noindent
The previous subsections explored Histograms, Bar Charts, and Kernel Density Estimation, which are univariate methods used for analyzing independent variables in isolation, revealing patterns and intrinsic properties within data. Now, the focus shifts to univariate methods of dependent variables, specifically Line Charts, which are crucial for visualizing relationships between variables to understanding their interactions and dependencies over time.
...
\begin{figure}[H]
<<echo=TRUE, message=FALSE, warning=FALSE, fig.pos = 'H', fig.dim = c(4, 3)>>=
...
GOING FROM ONE TO TWO POPULATIONS (two sample framework), BUT MAINTAINING ONE VARIABLE EACH 

After modeling and visualizing the relationship between two continuous variables, the focus shifts to assessing the performance of binary classification models. The Receiver Operating Characteristic (ROC) graph is a technique for assessing the performance of classification models as its discrimination threshold is varied.\\

\noindent
\textbf{Theory of ROC curve}\\
Let $Y$ be a binary variable indicating the true class of an instance, with $Y = 1$ for positive instances and $Y = 0$ for negative instances. Let $X$ be a continuous variable representing the predicted score or probability of an instance being classified as positive by the classifier.\\

\noindent
Since ROC graph plots two parameters\cite{rocAnalysis}: True Positive Rate (TPR, also known as Sensitivity, is the proportion of positive instances correctly identified) and False Positive Rate (FPR, 1 - Specificity, is the proportion of negative instances incorrectly identified as positive). Then define the TPR and the FPR as functions of a threshold $p$ in the interval $[0, 1]$, where $p$ denotes the probability threshold for classifying an instance as positive.\\

\noindent
TPR is defined as:
\[ \text{TPR}(p) = \frac{\text{TP}(p)}{\text{TP}(p) + \text{FN}(p)}, \]
where $\text{TP}(p)$ represents the number of true positives, and $ \text{FN}(p)$ represents the number of false negatives at threshold $p$.\\

\noindent
FPR is defined as:
\[ \text{FPR}(p) = \frac{\text{FP}(p)}{\text{FP}(p) + \text{TN}(p)}, \]
where $\text{FP}(p)$ is the number of false positives, and $\text{TN}(p)$ is the number of true negatives at threshold $p$.\\

\noindent
The ROC curve is then the set of points $(\text{FPR}(p), \text{TPR}(p))$ for all $p$ in the interval $[0, 1]$. The ROC curve can be represented as a parametric function of $p$, with $p$ varying from 0 to 1:

\[ \text{ROC} = \{ (\text{FPR}(p), \text{TPR}(p)) \,|\, p \in [0, 1] \} \]

\noindent
Also, the Area Under the Curve (AUC) provides a single measure of overall performance of a classifier. This definition represents the integral of the TPR $\text{TPR}(p)$ with respect to the FPR $\text{FPR}(p)$ over the interval from 0 to 1. It can be defined as:

\[ \text{AUC} = \int_{0}^{1} \text{TPR}(p) \quad d(\text{FPR}(p)) \]
...
<<animated-plot, echo=FALSE, fig.height=6, fig.width=6, fig.pos = 'H', fig.cap='Animated scatter plot: Horsepower vs MPG', message=FALSE>>=

# Basic scatter plot
p <- ggplot(mtcars, aes(x = mpg, y = wt)) +
  geom_point() +
  labs(title = 'Animated Scatter Plot', x = 'Miles per Gallon', y = 'Weight')

# Add animation
animated_plot <- p + transition_states(gear, transition_length = 2, state_length = 1) +
  enter_fade() +
  exit_fade()
...
\noindent Since each bubble \(B_i\) can be defined mathematically by the triplet \((x_i, y_i, C_i)\), akin to the scatter plot, the bubble plot can be described mathematically as a set of points \( \{B_1, \ldots, B_n\}\), where \(n\) represents the number of observations in the set.\\
...
<<estate-data-plot,fig.height=3,fig.width=4,fig.cap='Estate valuation using house age, linear regression and smooth regression methods', message=FALSE,warning=FALSE>>=
...
\noindent
\subsubsection{Theory of Generalised Linear Models}

\noindent
\\By definition a Generalised Linear Model has three components \cite{glm}: 
\begin{itemize}
...
\noindent Principal component biplots are based on singular value decomposition of the $(n \times p)$ data matrix $\mathbf{X}$ of $n$ observations on $p$ variables with rank $r$. SVD claims that 

\[\mathbf{X} = \mathbf{U}\mathbf{B}\mathbf{S}^{T}, \]

\noindent where $\mathbf{U}$ is an $(n \times r)$ orthogonal matrix, and $\mathbf{S}$ is an $(p \times r)$ orthogonal matrix. Furthermore, the columns of $\mathbf{U}$ and $\mathbf{S}$ are the so-called \textbf{left} and \textbf{right singular vectors}. $\mathbf{B}$ is a $(r \times r)$ diagonal matrix with entries $b_1, \ldots, b_r$ such that $b_1 \geq \dots b_r \geq 0$. These entries are known as the singular values.\\

\noindent EXPLAIN HOW IT IS OBTAINED \\

\noindent Having this decomposition of the data, it can be rewritten as
\[\mathbf{X} = \mathbf{U}\mathbf{B}\mathbf{S}^{T}= \mathbf{G}\mathbf{H}^{T},\]

\noindent where \[\mathbf{G} = [\mathbf{g}_1, \dots , \mathbf{g}_n] = \mathbf{U}\mathbf{B}^\alpha, \space \mathbf{H}^T = [\mathbf{h}_1, \dots , \mathbf{h}_p]^T = \mathbf{B}^{1-\alpha}\mathbf{S}^T, \]

\noindent and $\mathbf{B}^{a} \equiv \text{diag}(b_{1}^{a}}, \ldots, b_{r}^{a})$, where $a \in \mathbb{R}$.\\

\noindent Hence, the $i$th observation of the $j$th variable can be rewritten as

\[ x_{ij} = \mathbf{g}_i^T \mathbf{h}_j, \]

\noindent thus suggesting that if instead of using the $\mathbf{g}_i$ and the $\mathbf{h}_j$, we can just use their first elements (say, 2), respectively denoted by  $\mathbf{g}_{i}^{*}$  and  $\mathbf{h}_{j}^{*}$ , to get
...
\noindent \textbf{Understanding Biplots}\\

\noindent \textbf{Variable Vectors:} Each variable is represented as a vector originating from the origin of the plot. The direction and length of these vectors encode information about the relationships between variables.Closer vectors suggest higher correlations, while wider angles imply lower correlations. The direction of the vector corresponds to the orientation of the variable in the reduced-dimensional space, and the length reflects the variable's contribution to the overall variability of the data.\\

\noindent \textbf{Sample Points:} The position of each sample point in relation to the variable vectors conveys information about the relationships between observations and variables. The distance between sample points provides a measure of similarity or dissimilarity between observations.


<<PCAbiplot, fig.height=5.5, fig.width=8, fig.cap='PCA Biplot of mtcars Dataset: Principal Component Analysis', message=FALSE>>=
#GENERATION OF A BIPLOT
data(mtcars)
...
<<box-plot, echo=FALSE, fig.height=3, fig.width=5, fig.cap='Box Plot of Sepal Length by Species', message=FALSE>>=

my_palette <- c("#459395", "#EB7C69", "#FDA638")

# Create a box plot for sepal length by species with custom color palette
