...
<<scatter-plot-chunk, echo=FALSE, fig.height=5, fig.width=7>>=
...
<<buble-plot-chunk, echo=FALSE, fig.height=5, fig.width=9>>=
...
\subsection{3.4.1 Heatmaps}
Heatmaps are a versatile visualisation method, particularly effective at displaying matrix-like data. They excel in uncovering patterns, relationships, or variations within large datasets. Heatmaps present data as a grid of colored cells, where the color intensity of each cell represents the magnitude of the data it represents. This visual representation provides an immediate grasp of complex datasets.

\subsection{Heatmaps put to use}
...
%missing detailed analysis of visualisation created 
