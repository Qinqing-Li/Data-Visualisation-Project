...
\noindent Since each bubble \(B_i\) can be defined mathematically by the triplet \((x_i, y_i, C_i)\), akin to the scatter plot, the bubble plot can be described mathematically as a set of points \( \{B_1, \ldots, B_n\}\), specifically \( \{(x_1, y_1, C_1), \ldots, (x_n, y_n, C_n)\}\), where \(n\) represents the number of observations in the set.\\
...
correlation_matrix <- ggplot(correlation_matrix, aes(Var1, Var2, fill = Freq)) +
  ...
  scale_fill_gradient2(low = "#4B0082", mid = "white", high = "#FDA638", 
...
\noindent \text{Singular Value Decomposition}\\

\noindent Principal component biplots are based on singular value decomposition of the data matrix (n \times p) matrix $\mathbf{X}$ of $n$ observations on $p$ variables with rank $r$. SVD claims that 

\[\mathbf{X} = \mathbf{U}\mathbf{L}\mathbf{s}^{T}, \]
...
\[\mathbf{X} = \mathbf{U}\mathbf{L}\mathbf{S}^{T}= \mathbf{G}\mathbf{H}^{T}],\]

\noindent where \[\mathbf{G} = [\mathbf{g}_1, \dot , \mathbf{g}_n],  \mathbf{H}^T = [\mathbf{h}_1, \dot , \mathbf{h}_p]^T = $\mathbf{B}^{1-\alpha}\mathbf{S}^T, \]
...
\noindent thus suggesting that if instead of using the $\mathbf{g}_i$ and the $\mathbf{h}_j$, we can just use their first elements, respectively denoted by  $\mathbf{g}_{i}^{*}$  and  $\mathbf{h}_{j}^{*}$ , to get

\[ x_{ij} \approx \mathbf{g}_{i}^{*T}\mathbf{h}_{j}^{*}.\]

\noindent Thus, $\mathbf{g}_i$ and $\mathbf{h}_j$ should provide a reasonable two-dimensional approximation of the $n$ observations and the
$p$ variables.
