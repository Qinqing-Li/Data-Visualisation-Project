...
\noindent
The updated model estimate $\hat{g}(x)$, is computed using the local fitting method, but with the neighborhood weights $w_i{(x)$ replaced by $r_i w_i(x)$.\\

\noindent
\textbf{Case example: 1970s automobiles}\\

\noindent
Using the mtcars dataset, we explore the relations between maximum oil displacement (which is Engine Volume) and car efficiency (mearsured in Miles per Gallon), using two regression methods: LOESS and linear regression model, shown in Figure~\ref{fig:loess-plot}.\\

\noindent
The residual standard error of the LOESS model gives 2.828 and the residual standard error of the linear model is 3.251. One method to compare the goodness of fit between two models is cross validation.
<<loess-plot, echo=FALSE, fig.height=3, fig.width=4, fig.cap='LOESS regression model is indicated in red, linear regression model is indicated in blue.', message=FALSE>>=
# Obtain the order of rows based on the "disp" column
order_disp <- order(mtcars$disp)

# Reorder the mtcars dataset
mtcars_reordered <- mtcars[order_disp, ]

ggplot(data = mtcars_reordered, aes(x = disp, y = mpg)) +
  geom_point() +  # Scatterplot
  geom_smooth(method = "loess", se = FALSE, color = "#bd0026", span = 10) + 
  geom_smooth(method = "lm", se = FALSE, color = "#0868ac", formula = y ~ x) +
  labs(title = "Engine Volume (disp) vs. Miles per Gallon (mpg)", 
       x = "disp (cubic inches)", 
       y = "mpg") +
  theme_minimal() +
...
<<loess-plot-comment, echo=FALSE,message=FALSE,warning=FALSE>>=
# cannot perform F test as the data do not follow normality assumption: car types are manually selectted and enginee volume (disp) is certainly not normally distributed.

# cannot perform likelihood-ratio test, loess do not have log likelihood
@
\newpage
...
\subsection{Biplots}
\noindent 
Biplots serve as a valuable tool in visualising designed to project high-dimensional data onto a lower-dimensional space while retaining essential information. This technique, introduced by Herman Chernoff in the 1970s, aids in understanding the relationships and patterns within high-dimensional data. Biplots are commonly understood to be the mutivariate analogue of two-variable scatterplots.\\
...
\noindent In practical terms, the computational steps leading to the generation of a biplot involve: %https://eur02.safelinks.protection.outlook.com/ap/b-59584e83/?url=https%3A%2F%2Fuoe-my.sharepoint.com%2F%3Ab%3A%2Fg%2Fpersonal%2Fs2099120_ed_ac_uk%2FEWNG5FSX0fhLum4QCPr0kJYBhh_hjTO0fExMP2NYeljovA&data=05%7C02%7CP.Fullaondo%40sms.ed.ac.uk%7C9bf8d798a0594caf645908dc18fc3cd7%7C2e9f06b016694589878910a06934dc61%7C1%7C0%7C638412718818516401%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C3000%7C%7C%7C&sdata=wprd1%2BRyQ6w9hGpO0LMFey3MyIIdsXaovrqkHWRCUAI%3D&reserved=0
\begin{itemize}
    \item \textbf{Data Standardisation}: Standardising the data matrix by centering (substracting the mean) and scaling it to ensure each variable contributes equally.
    \item \textbf{Calculating Singular Value Decomposition}: Applying SVD to the standardised data matrix to obtain the singular values and vectors. This extracts the dimensions coneying most of the variability of the information included in the initial data. 
...
\noindent \textbf{Data Matrices}\\
\noindent Data matrices used to produce biplots can take 3 different forms. They are most commonly composed by cases x variables (where the cases are always assumed to be the row and the variables be the columns). But data matrices can also be composed by categories x categories (a cross tabulation), or by cases x categories (compositional data), and more.\\

\noindent \textbf{Data Standardisation}\\
\noindent Depending on the data matrix, different transformations may need to occur. Some of the ones that could take place involve: functional transformations such as logarithmic, sine, power, etc., matrix transformations such as observed/expected frequence ratio, odds ratio, logratio, etc., application of row and/or columns weights, (weighted) centering of rows and columns, double-centering, and normalisation of rows and columns.\\

\noindent \textbf {(Weighted)Singular Value Decomposition (SVD)}\\
\noindent We perform (Weighted) Singular Value Decomposition (SVD) via the eigendecomposition of a cross-product matrix whose order depends on the number of columns of $Z$ (i.e., $Z^TZ$), since the number of columns is usually less than the number of rows – if this is not so, then we will merely be calculating a larger than necessary matrix and using more computational time for the solution.\\

\noindent The most general situation we shall need is when there are weights $r$ for the rows and weights $c$ for the columns, with associated diagonal matrices $D_r$ and $D_c$. Defaults are $D_r = (1/n)I$ (each of the $n$ rows is weighted equally by $1/n$) and $D_c = I$.\\
...
\noindent There are three types of coordinates.\\
\noindent \textbf{Standard coordinates:} Rows: $F_s = D^{-1/2}_r U$, Columns: $G_s = D^{-1/2}_c V$\\
\noindent \textbf{Principal coordinates:} Rows: $F_p = Fs\Gamma$, Columns: $G_p = Gs\Gamma$\\
\noindent \textbf{“Canonical” coordinates:} Rows: $F_c = Fs\Gamma^{1/2}$, Columns: $G_c = Gs\Gamma^{1/2}$\\

\noindent Usual choices of coordinates give the following standard types of biplots or maps:
\begin{itemize}
  ...
  \item Symmetric map (not a biplot): plot $F_p$ and $G_p$.
...
<<biplot, echo=FALSE, fig.height=7, fig.width=10, fig.cap='PCA Biplot of mtcars Dataset: Principal Component Analysis', message=FALSE>>=
data(mtcars)
...
R provides us with a dedicated package for t-SNE analysis, \textit{Rtsne}, which allows us to conduct the analysis conveniently using the \textit{Rtsne} function. We will use \textbf{Palmer Penguin} dataset to make a tSNE plot in R.
...
The dashed lines enclose ellipses that represent the distribution of different penguin species in a t-SNE plot from the above figure~\ref{fig:t-SNE}.

Although t-SNE is a very powerful tool in data visualization, it indeed has its own limitations as well. It is important to select the appropriate method according to different requirments.
\newpage  
