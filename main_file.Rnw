...
\begin{figure}[htbp]
  ...
  \begin{minipage}[b]{0.45\linewidth}
    ...
    # Create a messy, busy plot with random sizes and meaninglesscolours, blue background, and white grid lines
...
<<data-plots, echo=FALSE, message=FALSE, fig.height=8, fig.width=12, fig.cap='Different types of visualisation according to the data type used'>>=
...
<<cogload-plot, echo=FALSE, fig.height=4, fig.width=10, fig.cap='High vs low cognitive load demand through the reduction of visual complexity'>>=
...
clear_plot <- ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("setosa" = "grey", "versicolor" = "grey", "virginica" = "#b2182b")) + 
...
Bar charts are a versatile tool for data visualization, commonly used to compare different categories. The vertical bar chart is the most familiar, displaying categories on the X-axis and their frequencies or counts on the Y-axis. Horizontal bar charts, rotated 90 degrees, are useful for lengthy category names or numerous categories, with categories on the Y-axis and frequencies on the X-axis. Multi-set or grouped bar charts allow for side-by-side comparisons of sub-groups within categories, available in both vertical and horizontal orientations. Stacked bar charts show classes of values divided into sub-classes, often color-coded, with each segment's size representing its frequency or count and the total bar length reflecting the cumulative total. While bar charts are known for their simplicity and ease of understanding, stacked bar charts can become confusing with too many categories. Overall, bar charts are an effective way to present and compare various types of data.
...
The bar chart depicts the relationship between tooth growth and different dosages of a vitamin, measured in milligrams per day. On the X-axis, three distinct levels of vitamin dosage are presented, while the Y-axis indicates the average tooth growth for each dosage. A key observation from the chart is a noticeable trend where increasing vitamin dosages correlate with increased tooth growth, suggesting that higher vitamin doses may enhance tooth growth.

<<barcharts, echo=FALSE, fig.height=3, fig.width=10, fig.cap='Visualizing Tooth Growth by Dosage: A Comparison of Grouped and Stacked Bar Chart Techniques'>>=
data(ToothGrowth)
...
<<bubble-plot, echo=FALSE, fig.height=5, fig.width=9, fig.caption='Bubble plot illustrating the relationship between car models and miles per gallon'>>=
#Create bubble plot 
