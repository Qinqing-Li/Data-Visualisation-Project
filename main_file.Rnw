...
<<scatter-plot, echo=FALSE, fig.height=4, fig.width=6, fig.cap='Scatter plot of car weights vs MPG'>>=
# Create a scatter plot of car weight vs MPG from mtcars dataset
library(ggplot2)
ggplot(data = mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  geom_point(size = 5, alpha = 0.7) +  # Larger points with transparency
...
\end{enumerate}
<<barchart1,echo=TRUE,fig.height=4,fig.width=6, fig.cap='Tooth growth by dose'>>=
library(datasets)
# load ToothGrowth dataset from R
data(ToothGrowth)
ggplot(ToothGrowth, aes(x = factor(dose), y = len)) +# Create a bar chart with different vitamin doses on the X-axis and tooth growth on the Y-axis (len).
...
<<barchart2, echo=5:13,fig.height=4, fig.width=6, fig.cap='Tooth Growth by Dose and Supplement(grouping bar chart)'>>=
...
\end{enumerate}
<<bar chat3, echo=3:11, fig.height=4, fig.width=6, fig.cap='Tooth Growth by Dose and Supplement(stacked bar chart)'>>=
filtered_data <- ToothGrowth %>% filter(dose %in% c(0.5, 1, 2))
...
<<spacetime-fy22,fig.height=5.2,fig.cap='Frequency of Fire by Space and Time, FY22',fig.show='hold', message=FALSE, warning=FALSE>>=
...
spacetime_fy22 <- grid.arrange(space_heatmap, time_heatmap, nrow = 2, heights = c(5,1))
...
<<fire-by-months-fy13-22,fig.height=3.5,fig.width=5.6,fig.cap='Frequency of Fire Occurrences, FY13-22',fig.show='hold', message=FALSE,warning=FALSE>>=
...
<<message=FALSE,warning=FALSE,fig.dim = c(6, 4), fig.cap='Exchange rates of CNY, CAN, EUR, HKD, USD to GBP'>>=
# Plot exchange rates of CNY, CAN, EUR, HKD, USD to GBP
plot_dt <- MyData %>% gather(key="Currency", value="Rate", -Date)

ggplot(plot_dt, aes(x=Date, y=Rate, color=Currency)) +
  geom_line() +
  labs(title="Exchange rates of CNY, CAN, EUR, HKD, USD to GBP",
...
<<message=FALSE,warning=FALSE,fig.dim = c(6, 4),fig.cap='21-day moving average of exchange rates of CNY, CAN, EUR, HKD, USD to GBP'>>=
...
# Plot 21-day moving average for each currency
...
ggplot(plot_data, aes(x=Date, y=Rate, color=Currency)) +
  geom_line() +
  labs(title="21-Day moving average of exchange rates",
       y="Exchange Rate to GBP",
       x="Date",
       color="Currency")
@
...
<<message=FALSE,warning=FALSE,fig.dim = c(6, 4),fig.cap='Decomposition of addictive time series model'>>=
...
<<message=FALSE,warning=FALSE,fig.dim = c(6, 4),fig.cap='Double y-axis time series'>>=
# Plot of double y-axis time series

# Plot the first time series with its y-axis
plot(MyData$CNYtoGBP, type="l", col="blue", ylab="Exchange Rate", xlab="Date",
...
<<message=FALSE,warning=FALSE,fig.dim = c(6, 4),fig.cap='Time series with anomalies'>>=
...
plot_data <- data.frame(Date =  MyData$Date,
...
<<warning=FALSE,message=FALSE,fig.dim=c(7,7), fig.caption='Crime numbers in London by boroughs'>>=
...
\end{itemize}
<<3d1, echo=TRUE, fig.cap='3d scatter plot'>>=
data(trees)
