...
\noindent
Graphical Statistics are an essential tool in the toolbox of every modern Data Scientist. Among other things, they allow their user to see through data, to explore it, as well as to communicate empirical findings in a visual way. This project will overview modern methods of data visualization, will study their theoretical properties, and will illustrate the power of these methodologies using synthetic as well as real data. Computational implementations via dashboards—using Python Dash or R Shiny—will be used to showcase the methodologies in practice. The project will keep in mind that the development of novel methods for visualising data is still an active field of research (e.g. Rodu and Kafadar, 2022), and thus will aim to cover both textbook methods as well as state-of-the-art approaches and open problems.\\

\noindent
This thesis aims to explore the theoretical foundations underlying various visualisation methodologies and demonstrate their application using real-world datasets. In particular, Chapters 3, 4, and 5 focus on standard Graphical Statistics implementations for univariate, bivariate, and multivariate datasets. Chapter 3 will discuss the Kernel Density Estimate, ROC Curve, and Time Series Analysis, showcasing their implementation using the tooth growth dataset and exchange rate dataset. Chapter 4 will focus on the topic of regression, discussing the theory of Simple Linear Regression followed by LOESS regression. Chapter 5 includes the introduction of the model selection method AIC and Multiple Linear Regression, followed by the theoretical foundation of biplots and t-SNE. Subsequent chapters will cover active research areas in Graphical Statistics and implementation of dashboards. \\


\noindent
R code chunks for each figure are separately copied into the \href{https://github.com/Qinqing-Li/Data-Visualisation-Project/tree/main/code}{GitHub code folder} for review. In this thesis, \textit{package name} is italicised, and \texttt{R command} is displayed in a teletype font.\\
...
\noindent
Chapter 1 will present case examples of the impact and misuses of data visualisations, followed by an introduction to R package \textit{ggplot2}, a list of datasets used in this thesis and the organisation of the thesis.
...
\begin{figure}[H]
    ...
    \caption{Incomplete Data Analysis, ``How to lie with statistics" by Miguel de Carvahol \cite{lie}}
    \label{fig:misuse1}
\end{figure}
...
R language holds a pivotal position in the field of data visualization. As a powerful tool for statistical analysis and graphic creation, R offers a wide array of functionalities to cater to a range of data visualization needs, from basic to advanced. In data visualisation, \textit{ggplot2} is the main tool used to create different plots capable of representing various types of dataset. The foundation of \textit{ggplot2} lies in the Grammar of Graphics, allowing the sequential construction of individual elements composing a graph, which are subsequently combined to create a unified graphical representation.\\

\noindent
As shown in subsequent sections, it is possible to achieve numerous visualisations rather effortlessly by utilising data in R, or even external data, together with \textit{ggplot2}.When using \textit{ggplot2}, the certain objects are used repeatedly. Some of these are: \texttt{geom}, \texttt{scale}, \texttt{coord}, \texttt{aes}, \texttt{stat}, \texttt{theme} \texttt{labs} and so on.\\


\noindent 
Some of the key tools in R mobilised in the creation of visual plots from datasets emcompass \cite{ggplot2}:
\noindent
\texttt{geom} refers to geometric objects, pivotal components within \textit{ggplot2} used to define the visual representation of data in a plot. Each \texttt{geom} function corresponds to a specific graphical representation type in a chart. \texttt{scales} facilitate the mapping of data onto the aesthetic attributes of a graphic, such as colour, size, and shape. In \textit{ggplot2}, scale functions enable the adjustment of various mapping details, including colour choices, label formatting, legend arrangement, and more. \texttt{coord} addresses the mapping of data coordinates onto the plane of the graphic. It provides axes and gridlines to aid in interpreting the graph. Various coordinate systems like Cartesian, polar, and map projections are available. \texttt{faceting} is a robust feature enabling the partitioning of a single plot into multiple plots based on factors present in the dataset. This functionality proves particularly beneficial for exploring and presenting data with multiple groups or categories. The \texttt{theme} function holds significance in customising the non-data elements of plots. The theme system within \textit{ggplot2} allows precise adjustment of aesthetic aspects such as fonts, labels, legends, and background colours. This tool is essential for enhancing plot readability and creating visually captivating graphics tailored to specific audiences or publication requirements.\\

\noindent
The R language is a powerful data analysis tool, and its power lies in the wide variety of R packages that help it achieve a wide variety of functions. Through out this project, we will use many different packages, such as $maps$, $dplyr$, $tidyverse$, $gridExtra$ and so on. These packages significantly enhances the analysis and presentation of data. \\

\noindent
The \textit{dplyr} packages plays a crucial role in data manipulation and transformation. It helps us easily filter, sort, summarize, and transform datasets, making data analysis more efficient and manageable. The \textit{maps} package is primarily used for providing map data and tools, allowing you to easily draw and display maps in geospatial analyses. The \textit{tidyverse} package is indeed a collection of R packages designed to provide a cohesive and comprehensive set of tools for data science. It includes several widely used packages such as \textit{ggplot2} and \textit{dplyr}. The \textit{gridExtra} packages offers the capability to combine several graphical objects into a cohesive display.\\

\noindent
Utilising these packages in your R project not only boosts efficiency but also ensures clarity and effectiveness in your data analysis and visualization tasks. Each package brings its unique strengths to the table, collectively enriching the R programming experience and enabling more sophisticated and comprehensive data analysis solutions.
\end{itemize}


\subsection{Datasets}
\noindent
The datasets that will be analysed and visualised in later chapters is introduced in this subsection.\\
...
<<data-plots, echo=FALSE, message=FALSE, fig.height=8, fig.width=11, fig.cap='Different types of visualisation according to the data type used'>>=
...
attendance_data <- data.frame(
  ...
  attendance = c(20, 22, 25, 27, 30, 32, 28, 26, 25, 28, 30)  # Replace this with your attendance numbers
)
...
plot4 <-ggplot(income_data, aes(x = income)) +
  ...
  labs(title = "Expected Income Distribution", x = "Income (£/month)", y = "Frequency") +
  theme_minimal()
...
\textbf{The Gestalt Principles \index{The Gestalt Principles}}\\
...
<<cogload-plot, echo=FALSE, fig.height=3, fig.width=10, fig.cap='High vs low cognitive load demand through the reduction of visual complexity'>>=
...
\section{Univariate Data Methods}
...
Let \(X_1, X_2, \ldots, X_n\) be independent and identically distributed random variables with finite mean \( \mu \) and variance \( \sigma^2 \). Then for any \( \epsilon > 0 \), 
\[ P\left(\left|\frac{1}{n}\sum_{i=1}^n X_i - \mu\right| < \epsilon\right) \rightarrow 1 \] 
as \( n \rightarrow \infty \).\\
Histograms can be used to visualize the convergence of sample means to the population mean.\\

Let \(X_1, X_2, \ldots, X_n\) be independent and identically distributed random variables with finite mean \( \mu \) and variance \( \sigma^2 \). Define the sample mean \( \bar{X} = \frac{1}{n}\sum_{i=1}^n X_i \). Then, as \( n \) becomes large, the variable 
...
<<barcharts, echo=FALSE, fig.height=3, fig.width=10, fig.cap='Visualising Tooth Growth by Dosage: A Comparison of Grouped and Stacked Bar Chart Techniques'>>=
data(ToothGrowth)
filtered_data <- ToothGrowth %>% filter(dose %in% c(0.5, 1, 2))

barchart_plot1 <- ggplot(filtered_data, aes(x = factor(dose), y = len, fill = supp)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  labs(x = "Dose (mg/day)", y = "Tooth Length (mm)", fill = "Supplement") +
  theme_minimal()

barchart_plot2 <- ggplot(filtered_data, aes(x = factor(dose), y = len, fill = supp)) +
...
write on ROC curve and visualisation example.
...
<<bubble-plot, echo=FALSE, fig.height=5, fig.width=9, fig.cap='Bubble plot illustrating trends in car types and performance', message=FALSE>>=
#Create bubble plot 
...
\subsection{multiple linear regrerssion}
...
\subsection{t-SNE}
theory and visualisation of t-SNE in 2D and 3D
