...
\noindent Since each bubble \(B_i\) can be mathematically described as a set of three values: \((x_i, y_i, C_i\), similarly to the scatter plot, the bubble plot can be mathematically described as a set of points: \( \{B_1, \ldots, B_n\}\), that is, \( \{(x_1, y_1, C_1), \ldots, (x_n, y_n, C_n)\}\), where \(n\) is the number of observations in the set.\\
...
ggplot(mtcars, aes(x = rownames(mtcars), y = mpg, size = hp, color = cyl)) +
  ...
  scale_color_gradient(low = "#e8c9fe", high = "#4B0082") +
  theme_minimal() +
...
<<bubble-plot-construction, echo=FALSE, fig.height=3.5, fig.width=11, fig.cap='Scatter plot deconstruction of Figure 16', message=FALSE>>= 

# Scatter plot 1: Car models vs. Miles per gallon
plot1 <- ggplot(mtcars, aes(x = rownames(mtcars), y = mpg)) +
  geom_point(color = "#459395", size =2) +
