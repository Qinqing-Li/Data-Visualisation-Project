...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `geom\_point()`:\\\#\# ! Problem while computing aesthetics.\\\#\# i Error occurred in the 1st layer.\\\#\# Caused by error in `FUN()`:\\\#\# ! object 'house\_age' not found}}\end{kframe}
\end{knitrout}
...
In this case, using the T-sne algorithm helps us to reduce the multidimensional numerical data of the penguin dataset to a 2-dimensional space, and to we can also using scatter plots and elliptical areas  to visualize the distribution of different penguin. The dashed lines enclose ellipses that represent the distribution of different penguin species in a t-SNE plot from the above figure~\ref{fig:t-SNE}.
