...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Generate random values for t}
\hlstd{t} \hlkwb{<-} \hlkwd{runif}\hlstd{(}\hlnum{100}\hlstd{,} \hlopt{-}\hlnum{1}\hlstd{,} \hlnum{1}\hlstd{)}

\hlcom{# Create the dataset x with two dimensions}
\hlstd{x} \hlkwb{<-} \hlkwd{cbind}\hlstd{(t, t}\hlopt{^}\hlnum{3}\hlstd{)} \hlopt{+} \hlkwd{rnorm}\hlstd{(}\hlnum{200}\hlstd{,} \hlkwc{sd} \hlstd{=} \hlnum{0.05}\hlstd{)}

\hlcom{# Rename the columns}
\hlkwd{colnames}\hlstd{(x)} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"dim1"}\hlstd{,} \hlstr{"dim2"}\hlstd{)}

\hlcom{# Set up a multi-paneled plotting window}
\hlkwd{par}\hlstd{(}\hlkwc{mfrow} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{2}\hlstd{))}

\hlcom{# Plot the first dataset}
\hlkwd{plot}\hlstd{(x,} \hlkwc{main} \hlstd{=} \hlstr{"Artificial Dataset 1"}\hlstd{,} \hlkwc{xlab} \hlstd{=} \hlstr{"Dimension 1"}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{"Dimension 2"}\hlstd{,} \hlkwc{col} \hlstd{=} \hlstr{"#459395"}\hlstd{)}

\hlcom{# Fit principal curve to the dataset}
\hlstd{fit} \hlkwb{<-} \hlkwd{principal_curve}\hlstd{(x)}

\hlcom{# Plot the principal curve along with the data points}
\hlkwd{plot}\hlstd{(fit,} \hlkwc{main} \hlstd{=} \hlstr{"Principal Curve"}\hlstd{,} \hlkwc{xlab} \hlstd{=} \hlstr{"Dimension 1"}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{"Dimension 2"}\hlstd{)}
\hlkwd{whiskers}\hlstd{(x, fit}\hlopt{$}\hlstd{s,} \hlkwc{col} \hlstd{=} \hlstr{"#EB7C69"}\hlstd{)}
\end{alltt}
\end{kframe}\begin{figure}[H]
...
\caption[Principal curve fitting illustration on artifical dataset]{Principal curve fitting illustration on artifical dataset}\label{fig:P-curves}
\end{figure}
